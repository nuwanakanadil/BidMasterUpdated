...
if (isset($_POST["submit"])) {
    ...
    if ($result->num_rows > 0) {
        ...
        if (password_verify($psw, $hashed_password_from_db)) { //verify hashed password and user entered password
            ...
            $sql = "DELETE From registeredbidder WHERE Username = ? AND Password = ? ";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ss", $username,$hashed_password_from_db);  // Bind parameters
            $stmt->execute();
            $result = $stmt->get_result();

            if($result !== true)
            {
                echo "<script>alert('Account Deleted Successful!');
                window.location.href = '../Signup/signup.html';</script>";
            }
            else
            {
                echo "<script>alert('Account Deleted Unsuccessful.Please try again!!');      
                </script>";
